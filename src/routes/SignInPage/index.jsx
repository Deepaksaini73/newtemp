...
const RegisterPage = () => {
  ...
  const onCollegeLogin = async (values, resetValues) => {
    if (!loading) {
      ...
      if (values.username.length !== 12) {
        setMessage({ text: "Please enter a valid college ID", title: "Error" });
        onOpen();
      } else if (!values.password) {
        setMessage({ text: "Please enter password", title: "Error" });
        onOpen();
      } else {
        setMessage({ text: "Login successful (frontend only)", title: "Success" });
        onOpen();
      }
              ...
              <form className="max-w-[20rem]" onSubmit={handleSubmit}>
                ...
                <div className="relative z-0 mb-8">
                  <input
                    type="text"
                    id="username"
                    className="block py-2.5 px-0 w-full text-xl text-white bg-transparent border-0 border-b-2 border-gray-300 focus:outline-none focus:ring-0 focus:border-[#0EA5E9] peer"
                    placeholder=" "
                    onChange={handleChange}
                    value={values.username}
                  />
                  <label
                    htmlFor="username"
                    className="absolute text-xl text-gray-300 duration-300 transform -translate-y-6 scale-75 top-3 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6"
                  >
                    College ID
                  </label>
                </div>
                ...
                <div className="flex justify-center">
                  {loading ? (
                    <Spinner thickness="4px" speed="0.65s" color="#0EA5E9" size="lg" />
                  ) : (
                    <button
                      type="submit"
                      className="relative inline-flex items-center justify-center px-4 py-2 border-2 border-[#0EA5E9] text-[1.2rem] text-[#0EA5E9] uppercase cursor-pointer rounded-[25px] transition-all duration-300"
                    >
                      <span className="absolute inset-0 flex items-center justify-center w-full h-full transition-all duration-300 -translate-x-full text-white bg-[#0EA5E9] group-hover:translate-x-0">
                        <BsArrowRight className="text-3xl" />
                      </span>
